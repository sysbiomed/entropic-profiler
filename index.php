...
function changeType(e){
	...
	var type=e.value;
	if(f.sending.value=="true") return;
...
function varValidate(v){
	...
	switch(v.name){
		...
		case 'vartext': // ok
			...
			if(seqlength<15) { error=true; break;}
		...
		case 'varl': // ok
			if(!f.varstudy[0].checked) break;
			if(parseInt(v.value)<3 || parseInt(v.value)>15) v.value=5;
		...
		case 'varmotif': // ok
			...
			if(v.value.length<3 || v.value.length>15) error=true;
			break;
		case 'varstartpos': // ok
...
<body bgcolor="aliceblue" style="font-family:sans-serif;" onload="document.forminput.varstudy[<?php if($hascookies && $cookiestudy=="motif") print("1"); else print("0"); ?>].click();document.forminput.submit.disabled='';document.forminput.vartype[<?php if($hascookies) print(3); else print(0); ?>].click();document.forminput.sending.value='false';">
<center>
...
<b>Local Renyi entropic profiles of DNA sequences</b></a>. BMC Bioinformatics 2007, 8:393.<br>
Fernandes F., Vinga S., Freitas A.T., Almeida J.S.: <a href="http://www.biomedcentral.com/1756-0500/2/72" style="cursor:help;">
<b>Entropic Profiler - detection of conservation in genomes using information theory</b></a>. BMC Research Notes 2009, 2:72.
</em>
...
<br><a href="<?php print("http://".$_SERVER['SERVER_NAME'].dirname($_SERVER['PHP_SELF'])."/source/epsource.zip"); ?>">Source code</a>
<br><a href="<?php print("http://".$_SERVER['SERVER_NAME'].dirname($_SERVER['PHP_SELF'])."/UserManual.pdf"); ?>">User manual</a>
<br><a href="<?php print("http://".$_SERVER['SERVER_NAME'].dirname($_SERVER['PHP_SELF'])."/GenomeLinks.txt"); ?>">Genomes for performance tests</a>
</small>
</font></p>
</fieldset>
</td>
...
<input type="text" name="varmotif" value="ACGT" id="varmotif" size="13" maxlength="15" align="middle" onkeypress="return basesonly(event);" onblur="varValidate(this);">
</div>
</td>
...
<label for="varl" style="cursor:help;" name="Length of the motif that ends in the specified position." onmouseover="opentooltip(this);" onmouseout="closetooltip();">Resolution Length</label>
</div>
...
<select name="varphi" id="varphi" disabled>
<?php
for($i=1;$i<=10;$i++){
	print("<option value='".$i."'");
	if($i==10) print(" selected");
	print(">".$i."</option>\n");
}
?>
</select>
<label for="varphi" style="cursor:help;" name="The higher the &Phi; value, the higher is the weight given to larger motifs." onmouseover="opentooltip(this);" onmouseout="closetooltip();">Smoothing Parameter</label>
</div>
<div id="acgtarea" style="display:none;">
<input type="button" value=" A " onclick="if(varmotif.value.length<15) varmotif.value=varmotif.value+'A'; varValidate(varmotif); varmotif.focus();">
<input type="button" value=" C " onclick="if(varmotif.value.length<15) varmotif.value=varmotif.value+'C'; varValidate(varmotif); varmotif.focus();">
<input type="button" value=" G " onclick="if(varmotif.value.length<15) varmotif.value=varmotif.value+'G'; varValidate(varmotif); varmotif.focus();">
<input type="button" value=" T " onclick="if(varmotif.value.length<15) varmotif.value=varmotif.value+'T'; varValidate(varmotif); varmotif.focus();">
...
<label for="varwindow" style="cursor:help;" name="Range of positions to study around the specified position." onmouseover="opentooltip(this);" onmouseout="closetooltip();">Window Length</label>
&nbsp;&nbsp;&nbsp;&nbsp;
<input type="checkbox" name="findmax" id="findmax" onclick="if(this.checked) varl.disabled=true; else varl.disabled=false;">
<label for="findmax" style="cursor:help;" name="Automatically detect the length of the motif that maximizes the E.P. value of the specified position." onmouseover="opentooltip(this);" onmouseout="closetooltip();">Find Max L</label>
</div>
...
<label for="varallpos" style="cursor:help;" name="Search for the motif on all positions of the sequence." onmouseover="opentooltip(this);" onmouseout="closetooltip();">Search entire sequence</label>
&nbsp;&nbsp;
<label for="varstartpos" style="cursor:help;" name="Position in the sequence to start the search." onmouseover="opentooltip(this);" onmouseout="closetooltip();">From:</label>
...
<label for="varendpos" style="cursor:help;" name="Position in the sequence to end the search." onmouseover="opentooltip(this);" onmouseout="closetooltip();">To:</label>
...
<input type="hidden" name="sending" value="false">
